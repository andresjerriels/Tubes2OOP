...
public class Peta implements Serializable {
    ...
    public void moveWildEngimons() throws Exception {
        ArrayList<Engimon> processed = new ArrayList<Engimon>();
        for (int i = 0; i < length; i++){
            for (int j = 0; j < width; j++){
                if(matriksPeta.get(i).get(j).containWildEngimon()){
                    if (!processed.contains(matriksPeta.get(i).get(j).getWildEngimon())) {
                        processed.add(matriksPeta.get(i).get(j).getWildEngimon());
